...
def _mk_tables(*, action_id: str) -> tuple[pa.Table, pa.Table, pa.Table]:
    # Two entities, three timepoints each -> ensures churn metrics are well-defined.
    evidence_rows = []
    decision_rows = []
    recon_rows = []
    ...
    for entity_id in ["e0", "e1"]:
        for t_idx in [0, 1, 2]:
            ...
            # decision_time stays constant per t_idx; no dependence on event stream in metrics.
...
def test_exp3_identity_matches_exp2_cost_metrics() -> None:
    # Policy choice doesn't matter for identity equivalence; keep it simple.
    ...
    # Exp3 baseline-vs-noshock deltas should be exactly 0 under identity shock.
