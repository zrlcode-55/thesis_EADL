...
### Context / intent (1–3 sentences)
Continue Seed Set B holdout (seeds 30–59) with the next 5 regime points (indices 45–49) under the same locked configs and metric code.
...
### Runs executed (artifact pointers)
- Command:
  - `.venv\\Scripts\\exp-suite.exe grid-run --sweep-prefix exp1_grid_v1__B_r1 --seed-start 30 --seed-end 59 --start-index 45 --limit-points 5 --resume`
- Git revision recorded in artifacts (from first completed sweep in this batch): `1aad146`
- Regime points intended (indices 45–49):
  - `cr0p20__sig0p50__cfa20p00__cws0p10`
  - `cr0p20__sig0p50__cfa5p00__cws0p05`
  - `cr0p20__sig0p50__cfa5p00__cws0p10`
  - `cr0p20__sig1p00__cfa10p00__cws0p05`
  - `cr0p20__sig1p00__cfa10p00__cws0p10`

### Data integrity / completeness
- This batch was started in `--resume` mode (idempotent with respect to completed runs).
- Status: in progress at time of writing. When complete, we will record the 5 sweep dirs and verify **90/90 FINALIZED** for each.
...
### 2026-01-22 — Experiment 1 (Grid v1): Seed Set B holdout (Batch 9: 5 regime points; indices 40–44) complete (EVAL)
...
### Context / intent (1–3 sentences)
Continue Seed Set B holdout (seeds 30–59) with the next 5 regime points (indices 40–44) under the same locked configs and metric code.
...
### Runs executed (artifact pointers)
- Command:
  - `.venv\\Scripts\\exp-suite.exe grid-run --sweep-prefix exp1_grid_v1__B_r1 --seed-start 30 --seed-end 59 --start-index 40 --limit-points 5 --resume`
- Git revision recorded in artifacts: `f1a6ffe`
- Sweep dirs (each 90/90, FINALIZED; each contains `sweep_manifest.json` and per-run `run_manifest.json`):
  - `artifacts/sweep_exp1_grid_v1__B_r1__cr0p20__sig0p25__cfa5p00__cws0p05/`
  - `artifacts/sweep_exp1_grid_v1__B_r1__cr0p20__sig0p25__cfa5p00__cws0p10/`
  - `artifacts/sweep_exp1_grid_v1__B_r1__cr0p20__sig0p50__cfa10p00__cws0p05/`
  - `artifacts/sweep_exp1_grid_v1__B_r1__cr0p20__sig0p50__cfa10p00__cws0p10/`
  - `artifacts/sweep_exp1_grid_v1__B_r1__cr0p20__sig0p50__cfa20p00__cws0p05/`
